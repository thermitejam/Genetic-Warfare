...
public class CreateStatBox : MonoBehaviour
{
    ...
    public void MoveButtonClicked()
    {
        ...
        {
            if (alienPosition == 1)
            { // If the alien is in the inventory, then when we click the button we want to move it to the floor
        ...
        } else if (scene == 2)
        {
            if (alienPosition == 1)
            {
                GameObject.Find("BreedingManager").GetComponent<BreedController>().AddToBreedingSlot(clickedAlienID);
                
            } else if (alienPosition == 3)
            {
                GameObject.Find("BreedingManager").GetComponent<BreedController>().RemoveFromBreedingSlot(clickedAlienID);
            }
        }
    }
